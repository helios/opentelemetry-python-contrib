...
class KafkaPropertiesExtractor(object):
...
def _wrap_send(tracer: Tracer, produce_hook: HookT) -> Callable:
    def _traced_send(func, instance, args, kwargs):
        ...
        with tracer.start_as_current_span(span_name, kind=trace.SpanKind.PRODUCER) as span:
            ...
            inject(headers, context=set_span_in_context(span), setter=_kafka_setter)
            try:
...
def _start_consume_span_with_extracted_context(tracer: Tracer, headers: List, topic: str) -> Span:
...
def _wrap_next(tracer: Tracer, consume_hook: HookT) -> Callable:
    def _traced_next(func, instance, args, kwargs):
        ...
        if current_span.is_recording() and current_span.name.startswith("kafka.consume"):
            current_span.end()
        ...
        if record:
            ...
            span = _start_consume_span_with_extracted_context(tracer, headers, topic)
