...
from opentelemetry.instrumentation.kafka.utils import (
    _wrap_send, _wrap_next, _get_span_name, _start_consume_span_with_extracted_context, _kafka_getter, _kafka_setter)
...
class TestUtils(TestCase):
    ...
    def test_wrap_send(self,
        ...
        args = [topic_name]
        headers = []
        kwargs = {"partition": 0, "headers": headers}
        expected_span_name = _get_span_name("send", topic_name)

        wrapped_send = _wrap_send(tracer, produce_hook)
        retval = wrapped_send(original_send_callback, kafka_producer, args, kwargs)

        extract_bootstrap_servers.assert_called_once_with(kafka_producer)
        extract_send_partition.assert_called_once_with(kafka_producer, args, kwargs)
        ...
        enrich_span.assert_called_once_with(
            span, extract_bootstrap_servers.return_value, topic_name, extract_send_partition.return_value)

        set_span_in_context.assert_called_once_with(span)
        context = set_span_in_context.return_value
        inject.assert_called_once_with(headers, context=context, setter=_kafka_setter)

        produce_hook.assert_called_once_with(span, args, kwargs)

        original_send_callback.assert_called_once_with(*args, **kwargs)
    ...
    @mock.patch("opentelemetry.instrumentation.kafka.utils._start_consume_span_with_extracted_context")
    @mock.patch("opentelemetry.instrumentation.kafka.utils._enrich_span")
    @mock.patch("opentelemetry.instrumentation.kafka.utils.KafkaPropertiesExtractor.extract_bootstrap_servers")
    def test_wrap_next(self,
                       extract_bootstrap_servers: mock.MagicMock,
                       enrich_span: mock.MagicMock,
        ...
        record = wrapped_next(original_next_callback, kafka_consumer, args, kwargs)
        ...
        original_next_callback.assert_called_once_with(*args, **kwargs)
        self.assertEqual(record, original_next_callback.return_value)

        start_consume_span_with_extracted_context.assert_called_once_with(tracer, record.headers, record.topic)
        ...
        consume_hook.assert_called_once_with(span, args, kwargs)


    @mock.patch("opentelemetry.context.attach")
    @mock.patch("opentelemetry.trace.set_span_in_context")
    @mock.patch("opentelemetry.propagate.extract")
    def test_start_consume_span_with_extracted_context(self,
        ...
        headers = []
        topic = "test_topic"
        expected_span_name = _get_span_name("consume", topic)

        _start_consume_span_with_extracted_context(tracer, headers, topic)

        extract.assert_called_once_with(headers, _kafka_getter)
        context = extract.return_value
        tracer.start_span.assert_called_once_with(expected_span_name, context=context, kind=SpanKind.CONSUMER)
